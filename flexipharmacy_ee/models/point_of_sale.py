...
class pos_order(models.Model):
    ...
    def check_order_delivery_type(self):
        if self.delivery_type == 'pending' and self.state == 'draft':
            action_id = self.env.ref('flexipharmacy_ee.action_pos_payment_flexipharmacy_ee')
            return {
                'name': action_id.name,
                'type': action_id.type,
    ...
    def _process_order(self, order):
        ...
        if order.get('customer_email') and res and res.reserved:
            try:
                template_id = self.env['ir.model.data'].get_object_reference('flexipharmacy_ee',
    ...
    def create(self, values):
        ...
        #             try:
        #                 template_id = self.env['ir.model.data'].get_object_reference('flexipharmacy_ee', 'email_template_pos_ereceipt')
        #                 template_obj = self.env['mail.template'].browse(template_id[1])
        ...
        member_lst = []
        tax = res.env['ir.config_parameter'].sudo().get_param('flexipharmacy_ee.pos_commission_with')
        ...
        # ORDER SYNC
        if not res.user_id.pos_user_type == 'cashier':
            notifications = []
            users = self.env['res.users'].search([])
            for user in users:
                if user.sales_persons:
                    for salesperson in user.sales_persons:
                        if salesperson.id == res.user_id.id:
                            session = self.env['pos.session'].sudo().search([('user_id', '=', user.id)], limit=1)
                            if session:
                                notifications.append(
                                    [(self._cr.dbname, 'lock.data', user.id), {'new_pos_order': res.read()}])
                                self.env['bus.bus'].sendmany(notifications)
        return res
    ...
    def _commission_calculation(self):
        return self.env['ir.config_parameter'].sudo().get_param(
            'flexipharmacy_ee.pos_commission_calculation') or ''

    def commission_based(self):
        return self.env['ir.config_parameter'].sudo().get_param('flexipharmacy_ee.pos_commission_based_on') or ''
