...
class sales_details_pdf_template(models.AbstractModel):
    _name = 'report.flexipharmacy_ee.sales_details_pdf_template'
    _description = 'Used to Store Value for Sales Detail PDF Report.'

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']. \
            _get_report_from_name('flexipharmacy_ee.sales_details_pdf_template')
    ...
    #     report = report_obj._get_report_from_name('flexipharmacy_ee.sales_details_pdf_template')
    #     docargs = {
    ...
    #         'data': data
    #     }
    #     return report_obj.render('flexipharmacy_ee.sales_details_pdf_template', docargs)


class pos_sales_report_pdf_template(models.AbstractModel):
    _name = 'report.flexipharmacy_ee.pos_sales_report_pdf_template'
    _description = 'Used to Store Value for POS Sales Report PDF.'

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']. \
            _get_report_from_name('flexipharmacy_ee.pos_sales_report_pdf_template')
