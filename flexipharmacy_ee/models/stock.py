...
class stock_location(models.Model):
    ...
    @api.model
    def get_shop_location_quantity(self, product_id):
        config_list = []
        final_list = []
        pos_config_obj = self.env['pos.config'].search([])
        for each in pos_config_obj:
            config_list.append({'shop': each.multi_shop_id.name, 'loc_id': each.multi_shop_id.location_id.id})

        product_obj = self.env['product.product'].browse(product_id)
        if pos_config_obj:
            for each in pos_config_obj:
                sql = """SELECT location_id, sum(quantity) As qty
                            FROM stock_quant AS sq
                            WHERE product_id = %s AND location_id in %s
                            GROUP BY location_id
                """ % (product_id,
                       " (%s) " % ','.join(map(str, [each.multi_shop_id.location_id.id for each in pos_config_obj])))
            self._cr.execute(sql)
            shop_qty = self._cr.dictfetchall()

            for each in config_list:
                flag = False
                for qty in shop_qty:
                    if each.get('loc_id') == qty.get('location_id'):
                        each.update({'qty': qty.get('qty') or 0})
                ...
                if not flag:
                    ...
                    final_list.append(each)
            total = 0
            for each in shop_qty:
                total = total + each.get('qty')
