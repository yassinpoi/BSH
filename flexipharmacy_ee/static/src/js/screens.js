odoo.define('flexipharmacy_ee.screens', function (require) {
    "use strict";
...
//            //brand Search
    ...
    screens.ClientListScreenWidget.include({
        ...
        save_client_details: function(partner) {
            var self = this;
            var fields = {};
            self.remove_customer_id = false;
            self.remove_customer_name = '';
            this.$('.client-details-contents .detail').each(function(idx,el){
                if(el.name == 'active'){
                    if($('.detail.active').prop('checked')==true){
                        fields[el.name] = true;
                    }else{
                        partner['active'] = false;
                        self.remove_customer_id = partner.id;
                        self.remove_customer_name = partner.name;
        ...
        saved_client_details: function(partner_id){
            ...
            var params = {
                            ...
                            fields: ['name','city','mobile','phone','street','email','country_id',
                                    'remaining_wallet_amount','birth_date','anniversary_date',
                                    'property_product_pricelist','vat','zip','credit_limit','barcode', 'active'],
                         }
            rpc.query(params, {async: false}).then(function(partner){
                ...
                if (partner) {
                    ...
                    if(partner && partner[0]){
                        ...
                        if(self.pos.partners && self.pos.partners[0]){
                            self.pos.partners = _.reject(self.pos.partners, function(customer){ return customer.id === self.remove_customer_id;});
                            self.display_client_details('show', self.pos.partners[0]);
                            self.render_list(self.pos.partners);
                        }
                    }
                } else {
        ...
        render_list: function(partners){
            ...
            for(var i = 0, len = Math.min(partners.length,1000); i < len; i++){
                if(partners[i] && partners[i].active){
...
//                self.payment_input(key);
        ...
        validate_order: function(force_validation) {
            var self = this;
            var order = self.pos.get_order();
            _.each(this.pos.get_order().get_paymentlines(), function(each){
                if(each.cashregister.journal){
                    order.set_credit_payment(each.cashregister.journal.is_credit)
                }
            })
...
//								order.remove_orderline(line);
    ...
    screens.ProductScreenWidget.include({
        ...
        start: function(){
            ...
            $('div#shop_location_qty').click(function(){
                var order = self.pos.get_order();
                var lines = order.get_orderlines();
                var orderLines = [];
                if(lines.length <= 0){
                ...
                if(order.get_selected_orderline()){
                    ...
                    var params = {
                        ...
                        method: 'get_shop_location_quantity',
                        args: [prod.id],
                    }
                    rpc.query(params, {async: false}).then(function(result){
                        if(result){
                            ...
                            _.each(prod_info, function (i) {
                                prod_info_data += "<tr>"+
                                "<td style='color:gray;font-weight: initial !important;padding:5px;text-align: left;padding-left: 15px;'>"+i['shop']+"</td>"+
                                "<td style='color:gray;font-weight: initial !important;padding:5px;text-align: right;padding-right: 15px;'>"+i['qty']+"</td>"+
                                "</tr>"
                            });
                            if(lines.length > 0){
                                self.gui.show_popup('product_location_qty_popup',{prod_info_data:prod_info_data,total_qty: total_qty});
                            }
                        }
                    }).fail(function(){
                        self.pos.db.notification('danger',"Connection lost");
